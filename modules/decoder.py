...
class ConvTransE(torch.nn.Module):
    ...
    def forward_cl(self, evolve_ent, evolve_rel, triplets):
        ...
        e2_embedded = embedded_all_evovle[triplets[:, 2]]
    ...
    def forward_hybrid(self, query_rep, candidate_rep, rel_rep, static_ent, triplets):
       #query_rep = query_rep + e1_static

        query_all = query_rep.unsqueeze(1)
        candidate_rep = F.tanh(candidate_rep)
        e1_static = candidate_rep[triplets[:, 0]].unsqueeze(1)

        batch_size = len(triplets)
        rel_embedded_evolve = rel_rep[triplets[:, 1]].unsqueeze(1) 
        #
        stacked_inputs = torch.cat([query_all + e1_static , rel_embedded_evolve], 1)
        stacked_inputs = self.bn0(stacked_inputs)
        x = self.inp_drop(stacked_inputs)
        x = self.conv1(x)
        x = self.bn1(x)
        ...
        if batch_size > 1:
            x = self.bn2(x)
        query = F.relu(x)
        x = torch.mm(query, candidate_rep.transpose(1, 0))
